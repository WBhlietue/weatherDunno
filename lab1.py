...
for i in range(len(data)):
    for j in range(len(data[i])):
        data[i][j] = GetTextJson(j)[data[i][j]]
    index+=1
...
def CalculateTotal(where=""):
    answer = 0
    if((len(where)) > 0):
        cur.execute(f"select {headers[-1]}, count(*) / sum(count(*)) ovre() from {tableName} where {where} group by {headers[-1]};")
    else:
        cur.execute(f"select {headers[-1]}, count(*) / sum(count(*)) over() from {tableName} group by {headers[-1]};")
    res = cur.fetchall()
